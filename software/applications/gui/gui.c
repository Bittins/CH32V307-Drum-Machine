...
void guiUpdate(struct MachineState* state);
uint8_t guiKBEventGet(void);
uint8_t guiQueueGet(struct MachineState** state);
...
void guiUpdate(struct MachineState* state)
{
    ...
    uint8_t i;
    // Set main row of 16 to brightness 1 if it has a beat
    for (i = 0; i < 16; i++)
    {
        if (state->patt_data[cur_patt].seq[cur_seq].page[cur_page].step[i].type)
        {
...
uint8_t guiQueueGet(struct MachineState** state)
{
    RT_ASSERT(rt_mq_recv(state_data_mq, state, sizeof(*state), 0) == RT_EOK);

    rt_mq_delete(state_data_mq);
}
